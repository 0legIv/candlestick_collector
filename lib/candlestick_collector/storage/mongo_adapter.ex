defmodule CandlestickCollector.Storage.MongoAdapter do
  @mongo_opts [pool_timeout: 60_000, timeout: 60_000]

  @mongo_pid :mongo

  def insert_one(collection, doc, opts \\ [retry: false, rescue_errors: []]) do
  ...
  def find(mongo_pid, collection, filter, opts \\ [retry: true]) do
    ...
    with %Mongo.Cursor{} = cursor <-
           Mongo.find(mongo_pid, collection, filter, Keyword.merge(opts, @mongo_opts)) do
  ...
  def find_one(mongo_pid, collection, filter, opts \\ [retry: true]) do
    ...
    with %Mongo.Cursor{} = cursor <-
           Mongo.find_one(mongo_pid, collection, filter, Keyword.merge(opts, @mongo_opts)) do
      cursor |> Enum.map(map_fn)
    end
  end
